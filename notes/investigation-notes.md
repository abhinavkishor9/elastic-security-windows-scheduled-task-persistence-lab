# Investigation Notes — Scheduled Task Persistence

## Investigation Summary

This investigation examined a controlled Windows Scheduled Task named `ElasticLab06` on `DESKTOP-9MMM37V`.

The task was created using `schtasks.exe` with an `ONLOGON` trigger and configured to launch `C:\Windows\System32\notepad.exe`.

The investigation used local PowerShell validation and Elastic endpoint telemetry to examine task creation, task configuration, manual execution, process telemetry, and remediation.

## Investigation Objectives

- Validate Scheduled Task creation.
- Examine the task trigger and action.
- Investigate `schtasks.exe` process telemetry.
- Correlate task creation and execution commands.
- Determine whether the configured executable appeared as an independent process event.
- Document telemetry limitations.
- Remove the controlled task after investigation.
...
The controlled activity demonstrates creation and execution of a Windows Scheduled Task.

## Conclusion

The investigation demonstrated that Scheduled Task configuration and `schtasks.exe` execution activity can be investigated from Elastic endpoint telemetry.

The strongest evidence came from combining local task metadata with Elastic command-line telemetry.

The absence of a standalone `notepad.exe` event was documented as a telemetry limitation rather than converted into an unsupported claim about execution.
