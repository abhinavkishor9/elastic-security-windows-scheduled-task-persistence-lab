# Troubleshooting Notes — Scheduled Task Persistence
...
### 1. Task Creation Is One Evidence Source

The creation command proves that the task was configured.

It does not automatically prove that the task's action produced a process event in the queried telemetry.

### 2. Task State Provides Context
...
help reconstruct the Scheduler state.

### 3. Task Management and Task Execution Are Different

These are different investigation stages:
...
```text
Task Scheduler processes action
```

Each stage can have different telemetry.

### 4. Missing Child Telemetry Must Be Documented

The absence of `notepad.exe` telemetry was recorded as an unknown rather than inferred.

### 5. Parent Processes Need Context

The parent of `schtasks.exe` was `pwsh.exe`.

That relationship does not automatically describe the process that the Scheduled Task was configured to launch.
...
rather than assuming that every configured action produced an observed process event.
